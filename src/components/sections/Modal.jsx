...
const Modal = ({ isOpen, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div className="relative bg-black p-5 rounded-lg w-3/5 max-h-4/5 overflow-y-auto h-3/4"> {/* Added max-h-4/5 for max height */}
                <button
                    className="sticky top-0 right-0 p-2 text-2xl font-bold"
                    onClick={onClose}
                >
                    <div className="rounded-full w-[30px] bg-red-800">
                        &times;
                    </div>
                </button>
                <div className="flex justify-center mb-4">
                    <img src="/logo-vertical-full.png" alt="Logo" className="h-48" />
                </div>
                <div className="mt-4">
